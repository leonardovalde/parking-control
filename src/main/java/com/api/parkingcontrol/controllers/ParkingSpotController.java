...
public class ParkingSpotController {
  ...
  public ResponseEntity<Object> saveParkingSpot(
    ...
    if (
      parkingSpotService.existsByLicensePlateCar(
        parkingSpotDto.getLicensePlateCar()
      )
    ) {
      return ResponseEntity
        .status(HttpStatus.CONFLICT)
        .body("Conflict:License plate car already in use");
    }
    if (
      parkingSpotService.existsByParkingSpotNumber(
        parkingSpotDto.getParkingSpotNumber()
      )
    ) {
      return ResponseEntity
        .status(HttpStatus.CONFLICT)
        .body("Conflict:Parking spot number already in use");
    }
    if (
      parkingSpotService.existsByApartmentAndBlock(
      ...
      return ResponseEntity
        .status(HttpStatus.CONFLICT)
        .body("Conflict:Apartment or block already in use");
    }
  ...
  public ResponseEntity<Page<ParkingSpotModel>> getAllParkingSpots(@PageableDefault(page = 0, size = 10, sort = "id", direction = Sort.Direction.ASC) Pageable pageable) {
    return ResponseEntity
      ...
      .body(parkingSpotService.getAll(pageable));
  }
  ...
  public ResponseEntity<Object> getOneParkingSpot(
    ...
    Optional<ParkingSpotModel> parkingSpotModelOptional = parkingSpotService.getById(
      id
    );
